    ...
    DBLayer
      { {-----------------------------------------------------------------------
        ...
        initializeWallet = \wid cp meta txs gp -> do
          ExceptT $ do
            ...
            when (isRight res) $ do
              ...
              void $
                modifyDBMaybe transactionsDBVar $ \(_txsOld, _ws) ->
        ...
        removeWallet = \wid -> do
          ...
          ExceptT $
            modifyDBMaybe transactionsDBVar $ \_ ->
              let delta = Just $ RemoveWallet wid
               in (delta, Right ())
          ExceptT $
            modifyDBMaybe transactionsDBVar $ \_ ->
              let delta = Just GarbageCollectTxWalletsHistory
               in (delta, Right ()),
        ...
        putLocalTxSubmission = \wid txid tx sl -> do
          ...
          ExceptT $
            modifyDBMaybe transactionsDBVar $
              \(_txsOld, ws) -> do
                case Map.lookup wid ws of
                  Nothing -> (Nothing, Left errNoSuchWallet)
                  Just (TxMetaHistory metas, _) -> case Map.lookup (TxId txid) metas of
        ...
        updatePendingTxForExpiry = \wid tip ->
          ExceptT $
            selectWallet wid >>= \case
              Nothing -> pure $ Left $ ErrNoSuchWallet wid
              Just _ -> modifyDBMaybe transactionsDBVar $ \_ ->
