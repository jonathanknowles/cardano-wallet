...
data TxRelationF (f :: Decoration) = TxRelationF
...
decorateWithTxOuts (TxHistoryF w) =
  let txouts :: Map TxOutKey (TxOut, [TxOutToken])
...
solveTxOut ::
  ...
  TxRelationF 'Without ->
  TxRelationF 'With
solveTxOut txOutMap TxRelationF {..} =
  TxRelationF
    { ins =
        decorateInputs
