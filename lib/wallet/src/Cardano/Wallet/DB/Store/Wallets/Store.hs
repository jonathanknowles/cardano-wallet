...
mkStoreWalletsMetaWithSubmissions =
  ...
  where
    ...
    update ::
      Map W.WalletId (TxMetaHistory, TxLocalSubmissionHistory) ->
      DeltaMap W.WalletId DeltaWalletsMetaWithSubmissions ->
      SqlPersistT IO ()
    ...
    load = runExceptT $ do
      wids <-
        lift $
          fmap (view #txMetaWalletId . entityVal)
            <$> selectList @TxMeta [] []
      subsWids <-
        lift $
          fmap (view #localTxSubmissionWalletId . entityVal)
            <$> selectList @LocalTxSubmission [] []
      fmap Map.fromList $
        forM (nub $ wids <> subsWids) $ \wid ->
          (wid,)
