...
postIcarusWallet ctx body = do
  postLegacyWallet ctx (rootXPrv, pwd) $ \wrk wid ->
    W.createIcarusWallet @(WorkerCtx ctx) @s @k
      wrk
      wid
      wName
      (rootXPrv, pwdP)
    ...
    withWorkerCtx ctx wid liftE liftE $ \wrk -> do
      ...
      case Shared.ready (getState cp) of
        ...
        Shared.Active _ -> do
          ...
          let runSelection outs =
                W.selectAssets @_ @_ @s @k @'CredFromScriptK
                  wrk
                  era
                  pp
                  selectAssetsParams
...
balanceTransaction ctx genChange (ApiT wid) body = do
  ...
  withWorkerCtx ctx wid liftE liftE $ \wrk -> do
    ...
    let balanceTx ::
        ...
        balanceTx partialTx =
          liftHandler $
            W.balanceTransaction @_ @IO @s @k @'CredFromKeyK
              wrk
              genChange
              (pp, nodePParams)
              ti
    ...
    anyShelleyTx <-
      maybeToHandler ErrByronTxNotSupported
        . asAnyShelleyBasedEra
    ...
    case body ^. #encoding of
      Just HexEncoded ->
        pure $
          ApiSerialisedTransaction
            (ApiT $ W.sealedTxFromCardano res)
            HexEncoded
      _ ->
        pure $
          ApiSerialisedTransaction
    ...
    apiTx =
      ApiTransaction
        { id = ApiT $ tx ^. #txId,
          ...
          metadata =
            TxMetadataWithSchema (tx ^. #txMetadataSchema)
              <$> tx ^. #txMetadata,
...
instance IsServerError ErrConstructTx where
  toServerError = \case
    ...
    ErrConstructTxSharedWalletPending ->
      apiError err403 SharedWalletPending $
        mconcat
...
instance IsServerError ErrBalanceTx where
  toServerError = \case
    ...
    ErrBalanceTxZeroAdaOutput ->
      apiError err501 BalanceTxZeroAdaOutput $
        T.unwords
    ...
    ErrBalanceTxInternalError (ErrUnderestimatedFee c _) ->
      apiError err500 BalanceTxUnderestimatedFee $
        T.unwords
    ...
    ErrBalanceTxMaxSizeLimitExceeded ->
      apiError err403 BalanceTxMaxSizeLimitExceeded $
        T.unwords
