...
utxoFromKeys keys utxoProp =
  let addresses :: [Address]
   ...
   in forAll (vectorOf (length txOuts) genTxIn `suchThat` isUnique) $
        \txIns -> do
    ...
    then do
      describe ("Alonzo and earlier - " <> show cEra) $ do
        ...
        estimateMaxInputsTests @IcarusKey
          era
          [(1, 73), (5, 69), (10, 65), (20, 57), (50, 29)]
    else -- Babbage era and later has slightly larger transactions, meaning slightly
...
balanceTransactionSpec = describe "balanceTransaction" $ do
  ...
  where
    ...
    balanceTx tx =
      flip evalRand (stdGenFromSeed testStdGenSeed) $
        runExceptT $
...
distributeSurplusSpec = do
  ...
  describe "distributeSurplusDelta" $ do
    let simplestFeePolicy =
          LinearFee $
            LinearFunction
              { intercept = 0,
                slope = 1
              }
    let mainnetFeePolicy =
          LinearFee $
            LinearFunction
              { intercept = 150_000,
                slope = 44
...
restrictResolution (PartialTx tx (Cardano.UTxO u) redeemers) =
  let u' = u `Map.restrictKeys` (inputsInTx tx)
   in PartialTx tx (Cardano.UTxO u') redeemers
  where
            ...
            case updateSealedTx tx noTxUpdate of
              ...
              Right tx' -> do
                if tx /= tx' && show tx == show tx'
                  ...
                  do
                    let newEncoding = convertToBase Base16 $ Cardano.serialiseToCBOR tx'
...
dummyTimeInterpreterWithHorizon horizon =
  let slotLength = 1
   ...
   in hoistTimeInterpreter (runIdentity . runExceptT) $
        mkTimeInterpreter
          nullTracer
...
shelleyBasedTxFromBytes bytes =
  let anyEraTx =
   ...
   in case asAnyShelleyBasedEra anyEraTx of
